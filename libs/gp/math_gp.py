...
def middle_func(individual, pset, fitness_func, args):
    ...
    function = gp.compile(exp, pset)
    return [fitness_func(function, *args)]

def math_gp(fitness_func, num_inputs, args_fitness=[], pop_size=5, num_gen=100, cxpb=0.5, mutpb=0.2, max_depth=5, verbose=True, int_const=None, float_const=None, start_depth=[1, 3], subtree_depth=[1, 2]):
    ...
    pset.addPrimitive(operators._neg_, 1)
    #pset.addPrimitive(operators._F_, 1)
    pset.addPrimitive(operators._mod_, 1)
    pset.addPrimitive(operators._sqrt_, 1)
    pset.addPrimitive(operators._log_, 1)
    #pset.addPrimitive(operators._pot_, 2)
    #pset.addPrimitive(operators._sin_, 1)
    #pset.addPrimitive(operators._cos_, 1)
    ...
    function = gp.compile(gp.PrimitiveTree(hof.items[0]), pset)
